...
const MiganObjectRemoval: React.FC = () => {
  ...
  const processImageWithMask = async () => {
    ...
    try {
      ...
      // Get the drawing canvas - access canvas element directly via ref if needed
      // or use export method
      ...
      if (!drawingCanvasRef.current) {
        throw new Error('Drawing canvas ref not available');
      }
      ...
      // Export the drawing from react-sketch-canvas as a data URL
      ...
      // Create a temporary canvas to draw the mask onto for processing
      const maskProcessingCanvas = document.createElement('canvas');
      maskProcessingCanvas.width = imageCanvas.width;
      ...
      // Optional: Show the mask alone for debugging
      console.log('Mask only drawing (from export): ', maskProcessingCanvas.toDataURL());

      // Draw mask on output first as a test (optional debugging step)
      ...
      if (outputCtx) {
        ...
        outputCtx.drawImage(maskProcessingCanvas, 0, 0); // Draw the processed mask canvas
      ...
      // Convert mask drawing to tensor using the maskProcessingCanvas
      const {uint8Array: maskArray, shape: maskShape} = maskCanvasToUint8Array(
        maskProcessingCanvas, // Use the canvas with the exported mask drawn on it
        imageCanvas.width, 
        imageCanvas.height
      );
