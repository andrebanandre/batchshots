...
export default function ImageFormatConvertorPage() {
  ...
  const handleFileChange = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      const processedFilesResults = await Promise.allSettled(
      ...
      const newImagesResults = await Promise.allSettled(
        processedFiles.map(async (file) => {
          try {
            ...
            setProcessingErrors((prev) => [
              ...prev,
              `Could not create image for ${file.name}: ${
                (error as Error).message || "Unknown error"
              }`,
            ]);
            return null;
      ...
      const newImages = newImagesResults
        .filter(
          (result): result is PromiseFulfilledResult<ImageFile> =>
            result.status === "fulfilled" && result.value !== null
        )
        .map((result) => {
  ...
  const handleConvertImages = async () => {
    ...
    try {
      ...
      for (let i = 0; i < images.length; i++) {
        ...
        try {
          ...
          if (convertedFile) {
            ...
            setProcessingErrors((prev) => [
              ...prev,
              `Failed to convert ${image.file.name}`,
            ]);
          }
        } catch (error) {
          ...
          setProcessingErrors((prev) => [
            ...prev,
            `Error converting ${image.file.name}: ${
              (error as Error).message || "Unknown error"
            }`,
          ]);
        }
      ...
      setProcessingErrors((prev) => [
        ...prev,
        `Conversion failed: ${(error as Error).message || "Unknown error"}`,
      ]);
  ...
  const getConvertButtonText = () => {
    if (isConverting) {
      ...
      return images.length > 1
        ? t("mainCard.actions.convertImagesTo", {
            format: selectedFormat.toUpperCase(),
          })
        : t("mainCard.actions.convertImageTo", {
            format: selectedFormat.toUpperCase(),
          });
    }
  };
